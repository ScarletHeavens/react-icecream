...
import {emptyCart, removeItem, plusItem, minusItem,clickedPaid} from '../Redux/actions/cartActionCreator';
import img from '../../scss/assets/img/loading_icecream.png';
import {Link} from 'react-router-dom';


const Cart = () => { 
  const dispatch = useDispatch();
  const {items, totalPrice, totalItems} = useSelector(({cart}) => cart);
  const addedItems = Object.keys(items).map(key => {
    return items[key].items[0];
  });

  const onEmptyCart = () => {
    if (window.confirm("Are you sure you want to empty the cart?")) dispatch(emptyCart())
  }
  const onRemoveItem = (id)=> {
    if (window.confirm("Are you sure you want to delete the flavour?")) dispatch(removeItem(id))
  };
  ...
  setTimeout(()=> dispatch(clickedPaid(false)),7000);
  setTimeout(()=> dispatch(emptyCart()), 5000);
  }
...
<path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
           ...
           {
             addedItems.map(obj => <CartItem  key = {obj.id} onRemove={onRemoveItem} name = {obj.name} 
            type= {obj.type} sugar= {obj.sugar} imageUrl = {obj.imageUrl} 
            totalPrice ={items[obj.id].totalPrice} totalCount={items[obj.id].items.length}
            id={obj.id} onMinus ={onMinusItem} onPlus = {onPlusItem}/>)
           }
           <div><img scr='#'/></div>
            </div>
            <div className="cart__bottom">
              <div className="cart__bottom-details">
                <span> Total: <b>{totalItems} scoops</b> </span>
                <span> Price: <b>{totalPrice}$</b> </span>
              </div>
...
<path d="M7 13L1 6.93015L6.86175 1" stroke="#D3D3D3" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
          ...
          : <div className="cart cart--empty">
            <h2>The cart is empty <icon>😢</icon></h2>
            <p>Most probably, you are still choosing the mouth watering flavors.
              Go to the main page to pick your poison.
            </p>
            <img alt='empty cart' src={img} />
